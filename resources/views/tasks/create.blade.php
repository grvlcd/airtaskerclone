...
@section('content')
    <div class="flex items-center w-4/6 m-auto bg-teal-lighter">
        <div class="w-full p-8 m-4 bg-white rounded shadow-lg">
            ...
            <form class="mb-4" action="{{ route('tasks.store') }}" method="post">
                @csrf
                <div class="flex flex-col mb-4">
                    <label class="mb-2 text-lg font-bold uppercase text-grey-darkest" for="title">Title</label>
                    <input class="px-3 py-2 border text-grey-darkest" type="text" name="title" id="title"
                        value="{{ old('title') }}">
                </div>
                <div class="flex flex-col mb-4">
                ...
                <div class="flex flex-col mb-4">
                    ...
                    <input class="px-3 py-2 border text-grey-darkest" type="number" min="10" step="1" name="rate" id="rate"
                        value="{{ old('rate') }}">
                </div>
                <div class="flex flex-col mb-6">
                    <label class="mb-2 text-lg font-bold uppercase text-grey-darkest" for="desired_data">Date
                        Execution</label>
                    <input class="px-3 py-2 border text-grey-darkest" type="date" value="{{ old('desired_daate') }}"
                        name="desired_date" id="desired_date">
                </div>
                <button
                    class="block px-5 py-2 mx-auto text-lg text-white uppercase bg-indigo-400 rounded bg-teal hover:bg-teal-dark"
