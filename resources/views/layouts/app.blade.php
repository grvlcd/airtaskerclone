...
<body>
            ...
            -->
                    ...
                    <div class="flex items-center justify-center flex-1 sm:items-stretch sm:justify-start">
                        ...
                        <div class="hidden sm:block sm:ml-6">
                            <div class="flex space-x-4">
                                ...
                                @auth
                                <a href="{{ route('tasks.index') }}" class="px-3 py-2 text-sm font-medium text-white rounded-md hover:bg-gray-700 hover:text-white">{{ __('Browse
                                        Task') }}</a>
                                <a role="button" href="{{route('tasks.create')}}" class="px-3 py-2 text-sm font-medium text-gray-300 rounded-md hover:bg-gray-700 hover:text-white">{{ __('Post') }}</a>
                                @endauth
                            </div>
                        </div>
                    </div>
                    <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                        ...
                        <div class="relative ml-3">
                            @guest
                            @if (Route::has('login'))
                            <a href="{{ route('login') }}" class="px-3 py-2 text-sm font-medium text-white rounded-md hover:bg-gray-700 hover:text-white">
                                {{ __('Login') }}</a>
                            @endif

                            @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="px-3 py-2 text-sm font-medium text-white rounded-md hover:bg-gray-700 hover:text-white">
                                {{ __('Register') }}</a>
                            @endif
                            @else
                            <div class="flex flex-row items-center justify-center">
                                ...
                                <div class="flex flex-row px-3 py-2 rounded-md hover:bg-gray-700 hover:text-white">
                                    ...
                                    <img class="inline w-8 h-8 rounded-full" src="{{ asset('images/' . Auth::user()->profile->path) }}">

                                    @else
                                    <img class="w-8 h-8 rounded-full" src="">
                                    {{-- https://i.pinimg.com/originals/df/28/37/df28378dbe3bc59d8e5d3646ade310b8.jpg --}}
                                    @endif
                                    <a href="{{ route('profile.index') }}" class="px-3 py-2 text-sm font-medium text-white rounded-md">
                                        {{ Auth::user()->name }}
                                    </a>
                                </div>


                                <a href=" {{ route('logout') }}" class="px-3 py-2 text-sm font-medium text-white rounded-md hover:bg-gray-700 hover:text-white" onclick="event.preventDefault();document.getElementById('logout-form').submit();">
                                    {{ __('Logout') }}
                                </a>
    ...
    <div class="relative w-1/2 px-6 py-4 m-auto mt-4 mb-4 text-white bg-green-500 border-0 rounded">
        ...
        <button class="absolute top-0 right-0 mt-4 mr-6 text-2xl font-semibold leading-none bg-transparent outline-none focus:outline-none">
            <span>×</span>
        </button>
    </div>
    @endif
